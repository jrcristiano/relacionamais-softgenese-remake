...
use App\Services\HistoryAcessoCardComprasService;

class AcessoCardShopping extends Award
{
    protected $awardRepo;
    protected $service;

    public function __construct(AwardRepo $awardRepo, AcessoCardShoppingService $service)
    {
    ...
    {
        $baseAcessoCardService = new BaseAcessoCardsCompraService(new BaseAcessoCardsCompraRepository(new BaseAcessoCardsCompra()));
        ...
        foreach ($documents as $key => $document) {
            if ($names[$key] != null) {
                ...
                $params['acesso_card_shopping_name'] = $acessoCardName && $acessoCardDocument == $formattedDocument ? $acessoCardName : $names[$key];
                $params['acesso_card_shopping_value'] = $values[$key];
                $params['acesso_card_shopping_number'] = $cardNumber;
                $params['acesso_card_shopping_proxy'] = $proxy;
                $params['acesso_card_shopping_document'] = str_pad($formattedDocument, 11, '0', STR_PAD_LEFT);
                $params['acesso_card_shopping_spreadsheet_line'] = $key + 1;
                $params['acesso_card_shopping_demand_id'] = $demandId;
                $params['acesso_card_shopping_award_id'] = $save->id;

                $status = $data['awarded_status'];

                if ($baseAcessoCardService->findBaseAcessoCardActiveByDocument($formattedDocument)) {
                    $params['acesso_card_shopping_already_exists'] = 1;
                }
                ...
                if ($findAcesso && !$statusCancelled && $status == 2) {
                    $this->service->saveByParam([
                        'acesso_card_shopping_number' => $params['acesso_card_shopping_number'],
                    ], 'acesso_card_shopping_document', $formattedDocument);
                }
                ...
                if ($findAcesso && $statusCancelled && !$statusActive && $status == 3) {
                    $params['acesso_card_shopping_number'] = null;
                    $this->service->save($params);
                }
                ...
                if ($findAcesso && $statusCancelled && $statusActive && $status == 3) {
                    $params['acesso_card_shopping_number'] = $activeCard->base_acesso_card_number;
                    $params['acesso_card_shopping_proxy'] = $activeCard->base_acesso_card_proxy;
                    $this->service->save($params);
                }
            }
        }
    ...
    public function updateCard(array $data)
    {
        ...
        $baseAcessoCardService = new BaseAcessoCardsCompraService(new BaseAcessoCardsCompraRepository(new BaseAcessoCardsCompra()));
        $historyAcessoCard = new HistoryAcessoCardComprasService(new HistoryAcessoCardComprasRepository(new HistoryAcessoCardCompra));
            ...
            foreach ($documents as $key => $document) {
                if ($names[$key]) {
                    ...
                    if (!$findBase) {
                        ...
                        $params = [];
                        $params['acesso_card_shopping_number'] = $baseAcessoCardNumber;
                        $params['acesso_card_shopping_proxy'] = $baseAcessoCardProxy;
                    ...
                    if ($findAcesso && !$statusCancelled && $statusActive) {
                        ...
                        foreach ($acessoCards as $acessoCard) {
                            if (!$acessoCard->acesso_card_proxy) {
                                ...
                                $this->service->saveByParam([
                                    'acesso_card_shopping_number' => $unlikedCard,
                                    'acesso_card_shopping_proxy' => $unlikedProxy,
                                ], 'acesso_card_shopping_proxy', $unlikedProxy);
                            }
                        }
                    }
                    ...
                    if ($findAcesso && $statusCancelled && !$statusActive) {
                        ...
                        foreach ($docs as $doc) {
                            $this->service->saveByParamWhereProxyNull([
                                'acesso_card_shopping_number' => $baseAcessoCardService->findActiveCardByDocument($doc)->base_acesso_card_number ?? null,
                                'acesso_card_shopping_proxy' => $baseAcessoCardService->findActiveCardByDocument($doc)->base_acesso_card_proxy ?? null,
                            ], 'acesso_card_shopping_document', $doc);
                        }
                    }
