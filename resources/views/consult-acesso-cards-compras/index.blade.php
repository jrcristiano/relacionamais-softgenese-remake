...
@section('title', 'Consulta de AcessoCards compras')
@section('content')
@php
    // dd($callCenters)
@endphp
    ...
    <div class="row shadow bg-white rounded">
        ...
        <div class="col-lg-10 sgi-container shadow-sm rounded p-0">
            <header class="sgi-content-header d-flex align-items-center">
                ...
                <h2 class="sgi-content-title">Cartões AcessoCards compras</h2>
                @php
                    $pedidoId = $id ?? null;
                @endphp
            ...
            <table id="client_table" class="table table-sm table-striped table-hover">
                ...
                <tbody>
                    @forelse ($awardeds as $awarded)
                        <tr>
                            <td class="text-uppercase">{{ $awarded->acesso_card_shopping_name }}</td>
                            <td class="text-uppercase">{{ $awarded->acesso_card_shopping_document }}</td>
                            ...
                            <td>
                                <a data-toggle="tooltip"
                                    data-placement="top"
                                    title="Visualizar"
                                    class="btn btn-sm btn-primary" href="{{ route('admin.operational.acesso-cards-compras.show', [
                                    'tipo_cartao' => 'compras',
                                    'premiado' => $awarded->acesso_card_shopping_name,
                                    'document' => $awarded->acesso_card_shopping_document,
                                    'acesso_card_shopping_id' => $awarded->acesso_card_shopping_id
                                ]) }}">
