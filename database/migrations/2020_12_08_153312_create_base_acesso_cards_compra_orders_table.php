...
class CreateBaseAcessoCardsCompraOrdersTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('base_acesso_cards_compra_orders', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->unsignedBigInteger('previous_card_id');
            $table->foreign('previous_card_id')->references('id')
                ->on('base_acesso_cards_compras');
    ...
    {
        Schema::dropIfExists('base_acesso_cards_compra_orders');
    }
}
